...
# OUTPUT_DIR, LOG_DIR, epochs, batch_size
    ...
    if test_dataset:
        ...
        with open(f"{args.LOG_DIR}/predictions.txt", "w") as f:
            for i in range(len(predictions)):
                f.write(predictions[i] + "\n")

        logging.info("Writing tokenized input to file...")
        with open(f"{args.LOG_DIR}/inputs.txt", "w") as f:
            for i in range(len(inputs)):
                f.write(inputs[i] + "\n")
        
        logging.info("Writing tokenized labels to file...")
        with open(f"{args.LOG_DIR}/references.txt", "w") as f:
